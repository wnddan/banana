...
class BlumBot:
    ...
    async def get_tg_web_data(self):
        ...
        async for message in self.client.get_chat_history('OfficialBananaBot'):
        ...
        if not start_command_found:
            await self.client.send_message("OfficialBananaBot", "/start referral=02UVAA")#ref_xavygoyfrvstgwv7gptymu
        web_view = await self.client.invoke(RequestWebView(
            peer=await self.client.resolve_peer('OfficialBananaBot'),
            bot=await self.client.resolve_peer('OfficialBananaBot'),
            platform='android',
            from_bot_menu=True,
